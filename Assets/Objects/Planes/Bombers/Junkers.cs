...
public class Junkers : GermanBomber
{
    ...
    void FireAtSpitfire()
    {
        ...
        if (isFiring == true)
        {
            if (timer <= 1.0f)
            {
                ...
                if (hitChance == 1)
                {
                    spitfireTarget.GetHitByJunkers();
                    spitfireTarget.RemoveFromChaserList(transform.root.gameObject);
                    fireTarget = transform.root.gameObject;
                }
            }
    ...
    void FireAtHurricane()
    {
        ...
        if (isFiring == true)
        {
            if (timer <= 1.0f)
            {
                ...
                if (hitChance == 1)
                {
                    hurricaneTarget.GetHitByJunkers();
                    hurricaneTarget.RemoveFromChaserList(transform.root.gameObject);
                    fireTarget = transform.root.gameObject;
                }
            }
    ...
    public void GetHitBySpitfire()
    {
        ...
        int pilotHitChance = Random.Range(1, 14); // chance to hit pilot
        if (pilotHitChance == 1)
        {
            pilot = 1;
            accuracy = accuracy - 1;
    ...
    public void GetDestroyed()
    {
        ...
        {
            Debug.Log("Junkers shot down");
        }
        if (missionComplete == true)
        {
            Debug.Log("Junkers Landed");
        }
    ...
    void Land()
    {
        ...
        airbaseScript.AddHeinkel(); // readd heinkel
        ScoreScreen score = GameObject.Find("ScoreScreen").GetComponent<ScoreScreen>();
        score.addBomber(-1);
        GetDestroyed();
